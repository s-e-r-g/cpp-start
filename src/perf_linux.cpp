...
struct PerfSession::Impl {
    struct Counter { int fd; std::string name; };
...
bool PerfSession::open(const std::vector<PerfCounterSpec>& specs){
    ...
    for(const auto& s: specs){
        ...
        if(fd == -1){
            // cleanup
            for(auto& c: impl_->counters) close(c.fd);
            delete impl_; impl_ = nullptr;
            return false;
        }
        if(group_fd == -1) group_fd = fd; // first counter is the group leader
